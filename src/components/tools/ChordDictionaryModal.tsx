// src/components/tools/ChordDictionaryModal.tsx
...
const ChordDictionaryModal: React.FC<ModalProps> = ({ chord, highlightVoicing, onClose }) => {
    ...
    return (
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} style={{ overflow: 'visible' }}>
        ...
        {voicing.frets.map((fret, stringIdx) => {
          ...
          return <circle key={`dot-${stringIdx}`} cx={x} cy={y} r="6" fill={isHighlighted ? "var(--accent-blue)" : "var(--accent)"} stroke="#000" strokeWidth="2" />;
        })}
      </svg>
    );
  };
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-color)', paddingBottom: '16px', flexShrink: 0 }}>
          <div>
            <div style={{ fontSize: '12px', color: 'var(--text-muted)', fontWeight: 800, letterSpacing: '1.5px', textTransform: 'uppercase' }}>
              // Chord Dictionary
            </div>
            <div style={{ fontSize: '32px', color: 'var(--text-primary)', fontWeight: 800 }}>
              {chord} <span style={{ color: 'var(--accent)' }}>Chord</span>
            </div>
          </div>
          <button 
            onClick={onClose} 
        ...
        {voicings.length > 0 ? (
          <div style={{ overflowY: 'auto', paddingRight: '10px' }}>
            <div style={{ fontSize: '13px', color: 'var(--text-secondary)', marginBottom: '20px', display: 'flex', justifyContent: 'space-between', flexShrink: 0 }}>
              <span>{highlightVoicing ? '🎯 Selected voicing:' : 'Available voicings & positions:'}</span>
              <span style={{ color: 'var(--accent)' }}>👆 Click to hear it!</span>
            </div>
              ...
              {voicings.map((v, i) => {
                ...
                return (
                  <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}>
                    <div style={{ 
                        fontSize: '12px', 
                        color: isHighlighted ? '#000' : 'var(--accent)', 
                        fontWeight: 800, 
                    ...
                    <div 
                      ...
                      style={{ 
                        ...
                        cursor: 'pointer', 
                        transition: 'all 0.2s', 
                        boxShadow: isHighlighted ? '0 0 20px rgba(0,255,157,0.15)' : '0 4px 12px rgba(0,0,0,0.3)' 
                      }}
                      onMouseEnter={(e) => { 
                      ...
                      onMouseLeave={(e) => { 
                        e.currentTarget.style.borderColor = isHighlighted ? 'var(--accent)' : 'var(--border-color)'; 
          ...
          <div style={{ padding: '40px 0', textAlign: 'center', color: 'var(--text-muted)' }}>
            ...
            <div style={{ fontSize: '16px', fontWeight: 600, color: 'var(--text-primary)' }}>
              No voicings found for {chord}
            </div>
            <div style={{ fontSize: '13px', marginTop: '8px', color: 'var(--text-muted)' }}>
              Try a different chord extension
            </div>
          </div>
        )}
      </div>
